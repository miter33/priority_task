...
public class CreateCustomerCommandHandler : IRequestHandler<CreateCustomerCommand, Customer>
{
    ...
    public Task<Customer> Handle(CreateCustomerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Name)) errors.Add("name", "Name is required.");
        if (string.IsNullOrWhiteSpace(request.Email)) errors.Add("email", "Email is required.");
        else if (!new EmailAddressAttribute().IsValid(request.Email)) errors.Add("email", "Email is invalid.");
        errors.ThrowIfAny();
